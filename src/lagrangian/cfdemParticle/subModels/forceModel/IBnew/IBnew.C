...
:
    ...
    useTorque_(false),
    interForceFieldName_(propsDict_.lookup("interForceFieldName")),
    Fs_(sm.mesh().lookupObject<volVectorField> (interForceFieldName_)),
    FInertial_(sm.mesh().lookupObject<volVectorField> ("FInertial")),
        ...
        //{
            ...
            for(int subCell=0;subCell<particleCloud_.cellsPerParticle()[index][0];subCell++)
            {
                ...
                if (cellI > -1) // particle Found
                {
                    ...
                    drag   += rho_[cellI]*(Fs_[cellI] - FInertial_[cellI])*Fs_.mesh().V()[cellI];
                    torque += (rc - positionCenter)^Fs_[cellI]*Fs_.mesh().V()[cellI]*rho_[cellI];
                }
